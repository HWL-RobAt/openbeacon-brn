...
// set broadcast MAC to 'SPTNK'
const unsigned char rfbroadcast_mac[NRF_MAX_MAC_SIZE]={'K','N','T','P','S'};
...
{
	nRFCMD_RegReadWrite(CONFIG | WRITE_REG,receive ? 0x3B : 0x3A);
}
...
void nRFAPI_TxRetries(unsigned char count)
{
	...
	nRFCMD_RegReadWrite(SETUP_RETR | WRITE_REG,0x10 | count);
}

void nRFAPI_PipesEnable(unsigned char mask)
{
	nRFCMD_RegReadWrite(EN_RXADDR | WRITE_REG,mask & 0x3F);
}

void nRFAPI_PipesAck(unsigned char mask)
{
	nRFCMD_RegReadWrite(EN_AA | WRITE_REG,mask & 0x3F);
}
...
unsigned char nRFAPI_SetSizeMac(unsigned char addr_size)
{
	...
	nRFCMD_RegReadWrite(SETUP_AW | WRITE_REG,addr_size);
...
{
	if(channel<=82)
		nRFCMD_RegReadWrite(RF_CH | WRITE_REG,channel);
}
...
{
	return nRFCMD_RegReadWrite(STATUS | WRITE_REG, status & MASK_IRQ_FLAGS);
}
...
{
	if(pipe<=5)
		nRFCMD_RegReadWrite((RX_PW_P0 + pipe) | WRITE_REG,size);
}
