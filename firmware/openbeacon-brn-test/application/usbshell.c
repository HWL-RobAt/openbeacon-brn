...
cmd_help (const portCHAR * cmd)
{
  ...
  shell_print ("RF-Status  gggggggg\n");
}
...
static void
cmd_rfstatus (const portCHAR * cmd)
{
  int val = 0;
  unsigned char c;
...
/*  int b;
  ...
  for (b = 0; b < 10; b++ ) 
    vUSBSendByte(packet[b]); 
*/
  //vUSBSendBuffer (packet, 0, 100);

  usb_send_buffer_zero_copy(packet, 1004, NULL, NULL, portMAX_DELAY);
...
usbshell_task (void *pvParameters)
{
  ...
  static struct cmd_t *n = commands; 
  n = NULL;
//  shell_print (PROMPT);
